...
/******************************************************************
  - Filesystem HTML Seite 
  - zugehörig die fs.html Datei muss ins Stammverzeichnis des Dateisystems des ESP
/**************************************************************************************/
...
bool handleList() {                                                                    // Senden aller Daten an den Client
  ...
  temp += ",{\"usedBytes\":\"" + formatBytes(fs_info.usedBytes) +                      // Berechnet den verwendeten Speicherplatz
          "\",\"totalBytes\":\"" + formatBytes(fs_info.totalBytes) +                   // Zeigt die Grösse des Speichers
...
bool handleFile(String &&path) {
  ...
  if (path == "/spiffs.html") sendResponce(); // VorrÃ¼bergehend fÃ¼r den Admin Tab
...
const String formatBytes(size_t const& bytes) {                                        // lesbare Anzeige der SpeichergrÃ¶ÃŸen
